...
meta.add_unlock = function(self, technology_name)
...
meta.remove_unlocked = function(self, technology_name)
    if type(technology_name) == "table" then
        for _, tech in pairs(technology_name) do
...
meta.add_ingredient = function(self, ingredient)
    for _, existing in pairs(self.ingredients) do
        if existing.name == ingredient.name and existing.type == ingredient.type then
            if existing.amount and ingredient.amount then
                ...
                existing.ignored_by_productivity = (existing.ignored_by_productivity or 0) +
                    (ingredient.ignored_by_productivity or 0)
                return self
            end
        end
    end
    ...
    table.insert(self.ingredients, ingredients)

    return self
end

meta.add_result = function(self, result)
    table.insert(self.results, result)
    return self
end

meta.remove_ingredient = function(self, ingredient_name)
...
meta.clear_ingredients = function(self)
    self.ingredient = {}
    return self
end

meta.multiply_result_amount = function(self, result_name, percent)
...
meta.multiplty_ingredient_amount = function(self, ingredient_name, percent)
...
meta.add_result_amount = function(self, result_name, increase)
    for _, result in pairs(self.results) do
        if result.name == result_name then
...
meta.add_ingredient_amount = function(self, ingredient_name, increase)
    for _, ingredient in pairs(self.ingredients) do
        if ingredient.name == ingredient_name then
