...
meta.add_prereq = function(self, prereq_technology_name)
    ...
    if not self.prerequisites then self.prerequisites = {} end
    table.insert(self.prerequisites, prereq_technology)
    return self
end

meta.remove_prereq = function(self, prereq_technology_name)
    if not self.prerequisites then return self end
    self.prerequisites = table.filter(self.prerequisites, function(prereq) return prereq ~= prereq_technology_name end)
    return self
end

meta.remove_pack = function(self, science_pack_name)
    if not self.unit then return self end
    self.unit.ingredients = table.filter(self.unit.ingredients,
        function(ingredient) return ingredient[1] ~= science_pack_name end)
    return self
end

meta.add_pack = function(self, science_pack_name)
...
TECHNOLOGY = setmetatable(data.raw.technology, {
    __call = function(self, technology)
        ...
        return setmetatable(technology, { __index = meta })
    end
})
